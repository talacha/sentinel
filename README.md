...
- **The admin console is powerful.** The console at `/admin` (enabled only by `ADMIN_PASSWORD`) can change where documents are sent, add other admins, and rewrite the policy reviews are judged against. It has a lockout against password guessing, but treat that password like root.
